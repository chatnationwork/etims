    ...
    <div className="grid grid-cols-3 gap-2">
      {actions.map((action) => (
        <button
          ...
          className={`flex flex-col items-center justify-center gap-1 py-3 px-2 rounded-lg text-center transition-colors ${action.color}`}
        >
          <action.icon className="w-5 h-5" />
          <span className="text-[10px] font-medium leading-tight">{action.title}</span>
        </button>
      ))}
    </div>
  );
